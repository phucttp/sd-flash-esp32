 ...
 * @brief Implementation cac hanh dong ung dung cap cao
 */
...
void app_actions_init(const app_actions_config_t* config) {
    ...
    s_status_text = "Ready";
    s_is_busy = false;
    ESP_LOGI(TAG, "App actions initialized, SD CS pin: %d", s_config.sd_cs_pin);
}
...
esp_err_t action_erase_chip_swd(void) {
    ...
    if (rdp_level == 0) {
        // Level 0 but might need rescue erase (zombie state: RDP=0xAA but flash has data)
        SHOW_PROGRESS("Verify L0...", 85);
        ret = flasher_swd_rdp_disable_verify();
        if (ret == ESP_OK) {
            s_progress = 100;
    ...
    // Step 2: Trigger RDP disable (writes OPTCR + waits ~12s for mass erase internally)
    SHOW_PROGRESS("RDP Disable...", 20);
    ret = flasher_swd_rdp_disable_trigger();
    if (ret != ESP_OK) {
        SHOW_MESSAGE("Error", "RDP Trigger Fail!");
        s_is_busy = false;
        return ret;
    }
    ...
    // Step 3: Ask user to POWER CYCLE STM32, wait for OK button
    SHOW_MESSAGE("POWER CYCLE", "STM32 (rut dien)");
    ESP_LOGW(TAG, ">>> Waiting for user to POWER CYCLE STM32...");

    // Wait for OK button press (blocking)
    if (s_config.btn_ok_pin >= 0) {
        // Wait for button release first (in case already pressed)
        ...
        // No button — just wait a fixed time
        vTaskDelay(pdMS_TO_TICKS(3000));
    }

    // Step 4: Verify RDP is now Level 0
